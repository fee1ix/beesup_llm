...
OBSERVATION_DESCRPTIONS_DICT={k:OBSERVATION_ATTRIBUTES_DICT[k]['desc'] for k in OBSERVATION_ATTRIBUTES_DICT.keys() if 'desc' in OBSERVATION_ATTRIBUTES_DICT[k]}

#PYDANTIC SCHEME PARSER
from pydantic import BaseModel, ValidationError, Field
from typing import Optional,List

class ObservationScheme(BaseModel):
...
class ReportScheme(BaseModel):
    ...
    observations: List[ObservationScheme] = None
...
""".strip()
    ...
    try:
        #ReportScheme.model_validate_json(completion)
        #completion_json=json.loads(completion)
        #completion_json=ReportScheme.model_validate_json(completion).json(exclude_none=exclude_none)

        completion_json=ReportScheme.parse_raw(completion).dict(exclude_none=exclude_none)
...
def parse_completion(completion,verbose=False):
    ...
    if pydantic_parse.is_valid==False:
        base_json={k:None for k in ReportScheme.model_fields.keys()}
...
def tabelize_json(base_json, create_meta_row=True):
    ...
    if not isinstance(base_json,dict):
        return pd.DataFrame(columns=ObservationScheme.model_fields.keys())
