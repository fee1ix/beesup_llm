...
class RAGPipeline:
    ...
    def __init__(
        ...
        if labh is not None:
            ...
            chunks_df=self.labh.handle_parameter(locals(),'chunks_df')
            llm_pipe=self.labh.handle_parameter(locals(),'llm_pipe')
            emb_pipe=self.labh.handle_parameter(locals(),'emb_pipe')
            limiters=self.labh.handle_parameter(locals(),'limiters')
