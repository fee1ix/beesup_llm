...
class BaseTrainer(BaseDirectory):
    type = 'trainer'

    @staticmethod
    def matches(ref):
        if getattr_or_key(ref, 'type') == ['training']: return True
        if SFTLoraTrainer.matches(ref): return True
        return False
    ...
    def from_ref(cls, ref=None, **kwargs):
        ...
        if SFTLoraTrainer.matches(pre_config):
            return SFTLoraTrainer(ref=pre_config, **kwargs)
...
class SFTLoraTrainer(BaseTrainer):


    @staticmethod
    def matches(ref):
        if hasattrs_or_keys(ref,['lora_config','sft_trainer_args']): return True
        return False
